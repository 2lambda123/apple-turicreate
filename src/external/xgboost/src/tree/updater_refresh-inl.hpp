...
class TreeRefresher: public IUpdater {
 public:
  ...
  virtual void Update(const std::vector<bst_gpair> &gpair,
    ...
    int nthread = turi::thread::cpu_count();
    // #pragma omp parallel
    {
    ...
    // #pragma omp parallel
    turi::in_parallel([&](size_t tid, size_t num_threads) {
...
#endif
    {
      ...
      while (iter->Next()) {
        ...
        turi::parallel_for (0, nbatch, [&](size_t i) {
          RowBatch::Inst inst = batch[i];
          // const int tid = omp_get_thread_num();
          const int tid = turi::thread::thread_id();
      ...
      turi::parallel_for (0, num_nodes, [&](size_t nid) {

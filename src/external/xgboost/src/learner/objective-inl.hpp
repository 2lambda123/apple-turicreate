...
class RegLossObj : public IObjFunction {
 public:
  ...
  virtual void PredTransform(std::vector<float> *io_preds) {
    ...
    turi::parallel_for (0, ndata, [&](size_t j) {
...
class PoissonRegression : public IObjFunction {
 public:
  ...
  virtual void GetGradient(const std::vector<float> &preds,
    ...
    turi::parallel_for (0, ndata, [&](size_t i) { // NOLINT(*)
      float p = preds[i];
  ...
  virtual void PredTransform(std::vector<float> *io_preds) {
    ...
    turi::parallel_for (0, ndata, [&](size_t j) {  // NOLINT(*)
...
class SoftmaxMultiClassObj : public IObjFunction {
 public:
  ...
  virtual void GetGradient(const std::vector<float> &preds,
    ...
    //   };
    // }
    size_t nthreads = turi::thread::cpu_count();
    turi::parallel_for (0, nthreads, [&](size_t thread_id) {

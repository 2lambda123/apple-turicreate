...
class edge_visitor_interface {
 public:
   ...
   */
  virtual void load_partition(sgraph& g, vertex_block<sframe>& source_vertex_block,
...
class triple_apply_impl {
 public:
  triple_apply_impl(sgraph& g, const std::vector<std::string>& mutated_vertex_fields,
                    const std::vector<std::string>& mutated_edge_fields,
                    bool requires_vertex_id = true)
    : m_graph(g)
    , m_requires_vertex_id(requires_vertex_id)
  {
 ...
 private:
   ...
   */
  template <typename EdgeVisitor>
  void do_work_on_edge_partition(sframe& edge_partition, edge_partition_address partition_address,
                                 EdgeVisitor visitor);
...
void triple_apply_impl::run(EdgeVisitor edge_visitor)
{
  ...
  std::function<void(std::vector<std::pair<size_t, size_t>>)> preamble_fn =
      [&](std::vector<std::pair<size_t, size_t>> coordinates) {
        ...
        for (const auto& coord : vertex_partition_to_load) message_ss << coord.partition << " ";
        logstream(LOG_INFO) << message_ss.str() << std::endl;
      };

  hilbert_blocked_parallel_for(
      m_graph.get_num_partitions(), preamble_fn, [&](std::pair<size_t, size_t> coordinate) {
...
void triple_apply_impl::init_data_structures(const std::vector<std::string>& mutated_vertex_fields,
                                             const std::vector<std::string>& mutated_edge_fields)
{
  ...
  std::set<std::string> vertex_field_set(all_vertex_fields.begin(), all_vertex_fields.end());
  std::set<std::string> edge_field_set(all_edge_fields.begin(), all_edge_fields.end());
 ...
 * This function will load all graph vertex blocks with the input_vetex_fields.
 */
void triple_apply_impl::load_graph_vertex_blocks(
 ...
 * mutated_vertex_fields.
 */
void triple_apply_impl::unload_graph_vertex_blocks(
    const std::set<vertex_partition_address>& vertex_address)
{
  std::vector<vertex_partition_address> address_vec(vertex_address.begin(), vertex_address.end());
  ...
  if (!m_mutated_vertex_fields.empty()) {
    ...
    parallel_for(0, address_vec.size(), [&](size_t i) {
      ...
      for (size_t i = 0; i < m_mutated_vertex_fields.size(); ++i) {
        ...
        old_vertex_data =
            old_vertex_data.replace_column(updated_vertex_data.select_column(i), column_name);
      }
  ...
  parallel_for(0, address_vec.size(),
               [&](size_t i) { m_vertex_data[address_vec[i].partition].unload(); });
}
                                                  ...
                                                  EdgeVisitor visitor)
{
  ...
  logstream(LOG_INFO) << "Do work on partition " << partition_address.partition1 << ", "
  ...
  mytimer.start();
  visitor.load_partition(m_graph, source_block, target_block, m_mutated_vertex_fields,
                         m_mutated_edge_fields, src_partition, dst_partition);
  ...
  logstream(LOG_INFO) << "Finish working on partition " << partition_address.partition1 << ", "
                      << partition_address.partition2 << " in " << mytimer.current_time() << " secs"
                      << std::endl;

  mytimer.start();
  // finalize the edge visitor
  visitor.finalize();
  logstream(LOG_INFO) << "Finalize working on partition " << partition_address.partition1 << ", "
                      << partition_address.partition2 << " in " << mytimer.current_time() << " secs"
                      << std::endl;
}
...
class single_edge_triple_apply_visitor : public edge_visitor_interface {
 public:
   ...
   */
  single_edge_triple_apply_visitor(triple_apply_fn_type apply_fn,
                                   std::vector<mutex_type>& lock_array, size_t srcid_column,
                                   size_t dstid_column)
    : apply_fn(apply_fn)
    , lock_array(lock_array)
    , srcid_column(srcid_column)
    , dstid_column(dstid_column)
  {
  }
   ...
   */
  void load_partition(sgraph& g, vertex_block<sframe>& source_vertex_block,
                      vertex_block<sframe>& target_vertex_block,
                      const std::vector<field_info>& _mutated_vertex_fields,
                      const std::vector<field_info>& _mutated_edge_fields, size_t _src_partition,
                      size_t _dst_partition)
  {
  ...
  void visit_edges(std::vector<edge_data>& edgedata)
  {
    ...
    for (auto& edata : edgedata) {
      ...
      edge_scope scope(&(*source_vertex_data)[srcid], &(*target_vertex_data)[dstid], &edata, lock_0,
                       lock_1);
  ...
  void finalize()
  {
    if (m_mutating_edge_data) {
      ...
      for (size_t i = 0; i < m_mutated_edge_field_ids.size(); ++i) {
        *edge_data_ptr = edge_data_ptr->replace_column(m_mutated_edges.select_column(i),
...
class batch_edge_triple_apply_visitor : public edge_visitor_interface {
 ...
 public:
   ...
   * Constructor
   */
  batch_edge_triple_apply_visitor(std::vector<mutex_type>& lock_array, size_t srcid_column,
                                  size_t dstid_column)
    : lock_array(lock_array)
   ...
   */
  void set_apply_fn(batch_triple_apply_fn_type _apply_fn) { apply_fn = _apply_fn; }
 ...
 private:
  ...
  std::vector<edge_scope>& get_locked_scopes()
  {
    ...
    for (const auto& idx_pair : m_locked_edge_idx) {
      ...
      m_locked_scopes.push_back(
          edge_scope(&(*source_vertex_data)[srcid], &(*target_vertex_data)[dstid], &edata));
    }
...
class lambda_triple_apply_visitor : public batch_edge_triple_apply_visitor {
 ...
 public:
  lambda_triple_apply_visitor(const std::string& lambda_str, std::vector<mutex_type>& lock_array,
                              size_t srcid_column, size_t dstid_column)
    : batch_edge_triple_apply_visitor(lock_array, srcid_column, dstid_column)
    , m_lambda_str(lambda_str)
  {
  ...
  lambda_triple_apply_visitor(const lambda_triple_apply_visitor& other)
    : batch_edge_triple_apply_visitor(other)
  {
  ...
  void load_partition(sgraph& g, vertex_block<sframe>& source_vertex_block,
                      vertex_block<sframe>& target_vertex_block,
                      const std::vector<field_info>& mutated_vertex_fields,
                      const std::vector<field_info>& mutated_edge_fields, size_t _src_partition,
                      size_t _dst_partition) override
  {
    // call parent function to initialize the internal data structrues.
    batch_edge_triple_apply_visitor::load_partition(g, source_vertex_block, target_vertex_block,
                                                    mutated_vertex_fields, mutated_edge_fields,
                                                    _src_partition, _dst_partition);

    for (auto& finfo : mutated_vertex_fields) m_mutated_vertex_field_ids.push_back(finfo.id);
    ...
    logstream(LOG_INFO) << "Acquire worker " << m_evaluator->id << " on partition "
                        << _src_partition << ", " << _dst_partition << std::endl;

    // Initialize evaluator graph compute with the graph information.
    try {
      m_evaluator->proxy->init(m_lambda_str, g.get_num_partitions(), g.get_vertex_fields(),
                               g.get_edge_fields(), m_srcid_column, m_dstid_column);
    } catch (cppipc::ipcexception& e) {
      throw(lambda::reinterpret_comm_failure(e));
    }

    // Load the vertex partitions for remote sgraph sync.
    logstream(LOG_INFO) << "Lambda worker load partition " << m_src_partition << ", "
                        << m_dst_partition << std::endl;
  ...
  void apply_lambda(std::vector<edge_scope>& edge_scopes)
  {
    ...
    if (m_mutating_edges) {
      ...
      std::copy(m_mutated_edge_field_ids.begin() + 2, m_mutated_edge_field_ids.end(),
                std::inserter(mutated_edge_field_ids, mutated_edge_field_ids.end()));
    }
    std::vector<sgraph_edge_data> mutated_edge_data;
    try {
      mutated_edge_data = m_evaluator->proxy->eval_triple_apply(
          all_edge_data, m_src_partition, m_dst_partition, mutated_edge_field_ids);
    } catch (cppipc::ipcexception& e) {
      throw(lambda::reinterpret_comm_failure(e));
    }
...
void triple_apply(sgraph& g, triple_apply_fn_type apply_fn,
                  ...
                  const std::vector<std::string>& mutated_edge_fields, bool requires_vertex_id)
{
                             ...
                             const std::vector<std::string>& mutated_edge_fields)
{
  ...
  batch_triple_apply_fn_type batch_apply_fn = [=](std::vector<edge_scope>& all_scope) {

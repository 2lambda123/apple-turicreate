...
sframe_index_file_information read_sframe_index_file(std::string index_file)
{
  ...
  try {
    ...
    ret.column_names = ini::read_sequence_section<std::string>(data, "column_names", ret.ncolumns);
    ret.column_files = ini::read_sequence_section<std::string>(data, "column_files", ret.ncolumns);
...
void write_sframe_index_file(std::string index_file, const sframe_index_file_information& info)
{
  ...
  if (info.column_names.size() != info.ncolumns || info.column_files.size() != info.ncolumns) {
  ...
  for (auto filename : info.column_files) {
    if (root_dir.length() > 0 && boost::algorithm::starts_with(filename, root_dir)) {

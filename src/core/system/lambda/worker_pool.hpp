                                                        ...
                                                        int connection_timeout)
{
  ...
  if (!fs::exists(the_path)) {
    throw std::string("Executable: ") + worker_binary + " not found.";
  }
  ...
  auto log_exception = [&](const std::string& e) {
    logstream(LOG_ERROR) << "Error starting CPPIPC connection in connecting to lambda worker at "
                         << worker_address << " (conn_time = " << conn_timer.current_time()
                         << "; attempt count = " << retry << "): " << e << std::endl;
  };
  ...
  while (true) {
    ...
    try {
      ...
      if (status == cppipc::reply_status::OK) {
        // Success
        logstream(LOG_INFO) << "Connected to worker " << new_process->get_pid() << " at "
                            << worker_address << "; conn_time = " << conn_timer.current_time()
                            << "; attempt count = " << retry << std::endl;
        ...
        logstream(LOG_ERROR) << "CPPIPC failure connecting to worker at " << worker_address
    ...
    if (LAMBDA_WORKER_CONNECTION_TIMEOUT >= 0 &&
      ...
      logstream(LOG_ERROR) << "Timeout connecting to lambda worker process"
                           << new_process->get_pid()
                           << "; conn attempt time = " << conn_timer.current_time()
                           << "; timeout = " << LAMBDA_WORKER_CONNECTION_TIMEOUT
                           << "; retry count = " << retry << std::endl;
      break;
    }
  }  // end of while
  ...
  // Done!
  logstream(LOG_INFO) << "Successfully launched lambda worker " << ret->id << std::endl;
  return ret;
}  // end spawn worker
 ...
 */
template <typename ProxyType>
std::unique_ptr<worker_process<ProxyType>> try_spawn_worker(
    std::vector<std::string> worker_binary_args, std::string worker_address,
...
class worker_pool {
 public:
   ...
   */
  std::shared_ptr<worker_guard<ProxyType>> get_worker_guard(
      std::unique_ptr<worker_process<ProxyType>>& worker)
  {
  ...
  void release_worker(std::unique_ptr<worker_process<ProxyType>>& worker)
  {
    ...
    if (check_alive(worker) == true) {
      ...
      // start new worker
      auto new_worker = try_spawn_worker<ProxyType>(m_worker_binary_and_args, new_worker_address(),
      ...
      if (new_worker != nullptr) {
        ...
        --m_num_workers;
        logstream(LOG_WARNING) << "Decrease number of workers to " << m_num_workers << std::endl;
      }
    }
    lck.unlock();
    cv.notify_one();
  ...
  std::vector<RetType> call_all_workers(Fn f)
  {
    ...
    try {
      ...
      for (auto& worker : temp_workers) guards.push_back(get_worker_guard(worker));
  ...
  /// constructor
  worker_pool(size_t num_workers, std::vector<std::string> worker_binary_and_args,
  ...
  ~worker_pool()
  {
    ...
    parallel_for(0, m_available_workers.size(), [&](size_t i) { m_available_workers[i].reset(); });
  }
 ...
 private:
   ...
   */
  std::string new_worker_address() { return "ipc://" + get_temp_name(); }
...
class worker_guard {
 public:
  worker_guard(worker_pool<ProxyType>* worker_pool_,
    ...
    : m_pool(worker_pool_)
    , m_worker(worker_process_)
  {
  }

  ~worker_guard() { m_pool->release_worker(m_worker); }

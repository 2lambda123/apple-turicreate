...
lambda_master& lambda_master::get_instance()
{
  if (instance_ptr == nullptr) {
    size_t num_workers =
        std::min<size_t>(DEFAULT_NUM_PYLAMBDA_WORKERS, std::max<size_t>(thread::cpu_count(), 1));
...
{
  m_worker_pool.reset(
      new worker_pool<lambda_evaluator_proxy>(nworkers, lambda_worker_binary_and_args));
  if (nworkers < thread::cpu_count()) {
    logprogress_stream << "Using default " << nworkers << " lambda workers.\n";
    logprogress_stream << "To maximize the degree of parallelism, add the following code to the "
                          "beginning of the program:\n";
    logprogress_stream
        << "\"turicreate.config.set_runtime_config(\'TURI_DEFAULT_NUM_PYLAMBDA_WORKERS\', "
        << thread::cpu_count() << ")\"\n";
    logprogress_stream
...
size_t lambda_master::make_lambda(const std::string& lambda_str)
{
  ...
  for (TURI_ATTRIBUTE_UNUSED_NDEBUG auto& v : returned_hashes) {
    DASSERT_MSG(lambda_hash == v, "workers should return the same lambda index");
  }
...
void lambda_master::bulk_eval(size_t lambda_hash, const std::vector<flexible_type>& args,
                              std::vector<flexible_type>& out, bool skip_undefined, uint64_t seed)
{
...
static bool shm_call(const std::shared_ptr<shmipc::client>& shmclient, oarchive& arguments,
                     RetType& ret)
{
  // send the message
  ...
  size_t receivelen = 0;
  arguments.buf = nullptr;
  shmok = shmipc::large_receive(*shmclient, &buf, &buflen, receivelen, (size_t)(-1));
  if (shmok == false) {
    free(buf);
    return false;
  }
 ...
 */
void lambda_master::bulk_eval(size_t lambda_hash, const sframe_rows& args,
                              std::vector<flexible_type>& out, bool skip_undefined, uint64_t seed)
{
  auto worker = m_worker_pool->get_worker();
  auto worker_guard = m_worker_pool->get_worker_guard(worker);
  ...
  try {
    ...
    if (shmclient_iter != m_shared_memory_worker_connections.end() &&
      ...
      oarchive oarc;
      oarc << (char)(bulk_eval_serialized_tag::BULK_EVAL_ROWS) << lambda_hash << args
           << skip_undefined << seed;
...
void lambda_master::bulk_eval(size_t lambda_hash, const std::vector<std::string>& keys,
...
void lambda_master::bulk_eval(size_t lambda_hash, const std::vector<std::string>& keys,
                              const sframe_rows& rows, std::vector<flexible_type>& out,
                              bool skip_undefined, uint64_t seed)
{
  ...
  try {
    ...
    if (shmclient_iter != m_shared_memory_worker_connections.end() &&
      ...
      auto& shmclient = shmclient_iter->second;
      oarchive oarc;
      oarc << (char)(bulk_eval_serialized_tag::BULK_EVAL_DICT_ROWS) << lambda_hash << keys << rows
           << skip_undefined << seed;
      bool good = shm_call(shmclient, oarc, out);
...
void set_pylambda_worker_binary_from_environment_variables()
{
  ...
  if (python_executable_env) {
    ...
    logstream(LOG_WARNING) << "Python executable not set. Python lambdas may not be available"
                           << std::endl;
  }

  std::string pylambda_worker_script;
  {
    boost::optional<std::string> pylambda_worker_script_env =
        turi::getenv_str("__GL_PYLAMBDA_SCRIPT__");
    if (pylambda_worker_script_env) {
      ...
      logstream(LOG_WARNING)
          << "Python lambda worker script not set. Python lambdas may not be available"
          << std::endl;
    }
  }

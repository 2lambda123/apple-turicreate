 ...
 */
std::shared_ptr<ngram_counter> save_and_load_model(
                                std::shared_ptr<ngram_counter> model) {

  std::shared_ptr<ngram_counter> loaded_model;
  loaded_model.reset(new ngram_counter);

  dir_archive archive_write;
  archive_write.open_directory_for_write("ngram_counter_tests");
  turi::oarchive oarc(archive_write);
  oarc << *model;
  archive_write.close();

  dir_archive archive_read;
  archive_read.open_directory_for_read("ngram_counter_tests");
 ...
 * Save the model and load it back.
 */
void check_model(std::shared_ptr<ngram_counter> model,
                 gl_sframe data,
                 std::map<std::string, flexible_type> opts) {
...
void run_ngram_counter_test(std::map<std::string, flexible_type> opts) {

  gl_sframe data = generate_data(opts);
  std::shared_ptr<ngram_counter> model = init_model(data, opts);
  std::shared_ptr<ngram_counter> loaded_model = save_and_load_model(model);
...
BOOST_FIXTURE_TEST_SUITE(_ngram_counter_test, ngram_counter_test)
BOOST_AUTO_TEST_CASE(test_ngram_counter_basic) {
  ngram_counter_test::test_ngram_counter_basic();
}
BOOST_AUTO_TEST_CASE(test_ngram_counter_ptb_tokenization) {
  ngram_counter_test::test_ngram_counter_ptb_tokenization();
}
BOOST_AUTO_TEST_CASE(test_character_ngram_counter_basic) {
  ngram_counter_test::test_character_ngram_counter_basic();
}
BOOST_AUTO_TEST_CASE(test_character_ngram_counter_variant) {
  ngram_counter_test::test_character_ngram_counter_variant();
}
BOOST_AUTO_TEST_CASE(test_bad_input_dict) {
  ngram_counter_test::test_bad_input_dict();
}
BOOST_AUTO_TEST_CASE(test_bad_input_list) {
  ngram_counter_test::test_bad_input_list();
}

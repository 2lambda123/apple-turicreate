 ...
 */
std::shared_ptr<word_counter> init_model(const gl_sframe& data,
                       std::map<std::string, flexible_type>& opts) {

  std::shared_ptr<word_counter> model;
  model.reset(new word_counter);
 ...
 */
std::shared_ptr<word_counter> save_and_load_model(
                                std::shared_ptr<word_counter> model) {

  std::shared_ptr<word_counter> loaded_model;
  loaded_model.reset(new word_counter);

  dir_archive archive_write;
  archive_write.open_directory_for_write("word_counter_tests");
  turi::oarchive oarc(archive_write);
  oarc << *model;
  archive_write.close();

  dir_archive archive_read;
  archive_read.open_directory_for_read("word_counter_tests");
 ...
 * Save the model and load it back.
 */
void check_model(std::shared_ptr<word_counter> model,
                 gl_sframe& data,
                 std::map<std::string, flexible_type>& opts) {
...
void run_word_counter_test(std::map<std::string, flexible_type>& opts) {

  gl_sframe data = generate_data(opts);
  std::shared_ptr<word_counter> model = init_model(data, opts);
  std::shared_ptr<word_counter> loaded_model = save_and_load_model(model);
...
struct word_counter_test  {
  ...
  void test_word_counter_basic() {
    std::map<std::string, flexible_type> opts = {
      ...
      {"delimiters", flex_list({"\r", "\v", "\n", "\f", "\t", " "})},
      {"exclude", false}};
    run_word_counter_test(opts);
  }

  void test_word_counter_ptb_tokenization() {
    std::map<std::string, flexible_type> opts = {
      {"examples", 20},
      {"to_lower", true},
...
BOOST_FIXTURE_TEST_SUITE(_word_counter_test, word_counter_test)
BOOST_AUTO_TEST_CASE(test_word_counter_basic) {
  word_counter_test::test_word_counter_basic();
}
BOOST_AUTO_TEST_CASE(test_word_counter_ptb_tokenization) {
  word_counter_test::test_word_counter_ptb_tokenization();
}
BOOST_AUTO_TEST_CASE(test_bad_input_dict) {
  word_counter_test::test_bad_input_dict();
}
BOOST_AUTO_TEST_CASE(test_bad_input_list) {
  word_counter_test::test_bad_input_list();
}
